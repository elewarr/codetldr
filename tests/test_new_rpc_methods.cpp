...
int main() {
    ...
    // ---- Test 9: get_control_flow with real cfg_nodes rows ----
    {
        // Insert 3 cfg_nodes rows for sym_compute
        {
            SQLite::Statement ins(db,
                "INSERT INTO cfg_nodes(file_id, symbol_id, node_type, condition, line, depth) "
                "VALUES (?, ?, ?, ?, ?, ?)");
            ins.bind(1, (long long)file_a);
            ins.bind(2, (long long)sym_compute);
            ins.bind(3, "if");
            ins.bind(4, "x > 0");
            ins.bind(5, 5);
            ins.bind(6, 0);
        ...
        }
        {
            ...
            ins.bind(2, (long long)sym_compute);
            ins.bind(3, "return");
            // NULL condition — bind nothing, use bindNull
            ins.bind(4); // bind null
            ins.bind(5, 14);
            ins.bind(6, 0);
            ins.exec();
        }

        auto result = builder.get_control_flow(sym_compute);
        CHECK(result.is_array(), "get_control_flow: result is JSON array");
        CHECK(result.size() == 3, "get_control_flow: 3 nodes for sym_compute");
        CHECK(result[0]["type"].get<std::string>() == "if",
              "get_control_flow: first node type=if");
        CHECK(result[0]["condition"].get<std::string>() == "x > 0",
              "get_control_flow: first node condition=x > 0");
        CHECK(result[0]["line"].get<int>() == 5,
              "get_control_flow: first node line=5");
        CHECK(result[0]["depth"].get<int>() == 0,
              "get_control_flow: first node depth=0");
        CHECK(result[1]["type"].get<std::string>() == "for",
              "get_control_flow: second node type=for");
        CHECK(result[1]["condition"].get<std::string>() == "i < n",
              "get_control_flow: second node condition=i < n");
        CHECK(result[1]["line"].get<int>() == 8,
              "get_control_flow: second node line=8");
        CHECK(result[2]["type"].get<std::string>() == "return",
              "get_control_flow: third node type=return");
        CHECK(result[2]["condition"].get<std::string>() == "",
              "get_control_flow: third node condition=empty (COALESCE NULL)");
        CHECK(result[2]["line"].get<int>() == 14,
              "get_control_flow: third node line=14");

        // Empty result for sym_helper (no cfg_nodes rows)
        auto empty_result = builder.get_control_flow(sym_helper);
        CHECK(empty_result.is_array(), "get_control_flow: empty result is array");
        CHECK(empty_result.empty(), "get_control_flow: sym_helper returns empty array");
    }

    // ---- Test 10: get_data_flow with real dfg_edges rows ----
    {
        // Insert 2 dfg_edges rows for sym_compute
        {
            SQLite::Statement ins(db,
        ...
        }
        {
            ...
            ins.bind(3, "return");
            ins.bind(4, "result");
            ins.bind(5); // NULL rhs_snippet
            ins.bind(6, 14);
            ins.exec();
        }

        auto result = builder.get_data_flow(sym_compute);
        CHECK(result.is_array(), "get_data_flow: result is JSON array");
        CHECK(result.size() == 2, "get_data_flow: 2 edges for sym_compute");
        CHECK(result[0]["type"].get<std::string>() == "assign",
              "get_data_flow: first edge type=assign");
        CHECK(result[0]["lhs"].get<std::string>() == "result",
              "get_data_flow: first edge lhs=result");
        CHECK(result[0]["rhs_snippet"].get<std::string>() == "x * 2",
              "get_data_flow: first edge rhs_snippet=x * 2");
        CHECK(result[0]["line"].get<int>() == 6,
              "get_data_flow: first edge line=6");
        CHECK(result[1]["type"].get<std::string>() == "return",
              "get_data_flow: second edge type=return");
        CHECK(result[1]["lhs"].get<std::string>() == "result",
              "get_data_flow: second edge lhs=result");
        CHECK(result[1]["rhs_snippet"].get<std::string>() == "",
              "get_data_flow: second edge rhs_snippet=empty (COALESCE NULL)");
        CHECK(result[1]["line"].get<int>() == 14,
              "get_data_flow: second edge line=14");

        // Empty result for sym_helper (no dfg_edges rows)
        auto empty_result = builder.get_data_flow(sym_helper);
        CHECK(empty_result.is_array(), "get_data_flow: empty result is array");
        CHECK(empty_result.empty(), "get_data_flow: sym_helper returns empty array");
    }
